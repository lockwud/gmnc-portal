...
import { Calendar, Clock, Shield, Bell, Save, Eye, ChevronLeft } from 'lucide-react';
import Link from 'next/link';
import Modal from '@/components/ui/Modal';
import Button from '@/components/ui/Button';
import { getProviderAppointmentSettings, updateProviderAppointmentSettings, PROVIDER_DEFAULT_SETTINGS } from '@/lib/api/settings';
import type { ProviderAppointmentSettingsType } from '@/lib/api/settings';
...
function CurrentSettingsCard({ settings }: { settings: ProviderAppointmentSettingsType }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-6">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-slate-900">Your Appointment Settings</h3>
          <p className="mt-1 text-xs text-slate-500">These settings control how caregivers book appointments with you</p>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <p className="text-xs text-slate-400">Patient Booking</p>
            ...
            <div className="space-y-2">
              <p className="text-xs text-slate-400">Buffer Time</p>
              <p className="text-sm font-medium text-slate-900">
            ...
            <div className="space-y-2">
              <p className="text-xs text-slate-400">Require Confirmation</p>
              <p className="text-sm font-medium text-slate-900">
...
export default function ProviderAppointmentSettings() {
  const [settings, setSettings] = useState<ProviderAppointmentSettingsType>({ ...PROVIDER_DEFAULT_SETTINGS });
  ...
  useEffect(() => {
    async function loadSettings() {
      try {
        setIsLoading(true);
        const data = await getProviderAppointmentSettings();
        if (data) {
          setSettings({ ...PROVIDER_DEFAULT_SETTINGS, ...data });
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await updateProviderAppointmentSettings(settings);
      setIsModalOpen(true);
    } catch (err) {
    ...
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">Loading your settings...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full pb-8 pt-4">
      <div className="w-full px-6">
        <header className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-semibold text-slate-900">My Appointment Settings</h1>
            <p className="mt-1 text-sm text-slate-500">
              Configure your personal appointment scheduling rules. Caregivers will see these settings when booking with you.
            </p>
          </div>
        </header>
        ...
        <div className="flex h-[calc(100vh-200px)] flex-col">
          <div className="flex-1 overflow-y-auto scrollbar-none">
            <div className="space-y-6 pb-4">
              ...
              <SettingCard title="Booking Preferences" description="Control how patients/caregivers can schedule with you" icon={Calendar}>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-medium text-slate-700">Allow Patient Booking</p>
                      <p className="text-xs text-slate-400">Let caregivers self-schedule online with you</p>
                    </div>
                    <ToggleSwitch
              ...
              <SettingCard title="Scheduling Rules" description="Configure duration, buffer, and limits" icon={Clock}>
                <div className="space-y-4">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-slate-700">Slot Interval</p>
                    <NumberInput
                      value={settings.slotInterval}
                      onChange={(val) => setSettings({ ...settings, slotInterval: val })}
                      min={5}
                      max={120}
                      unit="minutes"
                    />
                    <p className="text-xs text-slate-400">Time between available booking slots</p>
                  </div>
                </div>
              </SettingCard>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Appointments need your approval</p>
                    </div>
                    <ToggleSwitch
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">Allow caregivers to join waitlist when you are fully booked</p>
                    </div>
                    <ToggleSwitch
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Your appointment settings have been updated successfully. Caregivers will now see these preferences when booking with you.</p>
            </div>
          </div>
