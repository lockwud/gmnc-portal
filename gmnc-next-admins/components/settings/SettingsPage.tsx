...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
              ...
              >
                <Card className="p-8 border-none shadow-sm flex items-center justify-between gap-6">
                  <div className="flex items-center gap-6">
                    <div className="w-20 h-20 rounded-3xl bg-slate-100 flex items-center justify-center relative overflow-hidden group">
                      <Image
                        src="https://ui-avatars.com/api/?name=Mubarak+Salisu&background=random"
                        alt="Profile"
                        fill
                        sizes="80px"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button
                    ...
                    className="p-6 rounded-4xl border border-slate-100 bg-white hover:border-emerald-100 transition-all text-left flex items-center justify-between group"
                  >
                    <div className="space-y-1">
                      <p className="font-bold text-primary">Change password</p>
                      <p className="text-[10px] text-slate-400 max-w-35">At least 8 characters with at least a symbol and a number.</p>
                    </div>
                  ...
                  <button className="p-6 rounded-4xl border border-slate-100 bg-white hover:border-rose-100 transition-all text-left flex items-center justify-between group">
                    <div className="space-y-1">
      ...
      <AnimatePresence>
        {showAddUser && (
          <SettingsModal title="Add User" onClose={() => setShowAddUser(false)}>
            <form className="space-y-5">
              <Input label="Email" placeholder="Enter user email" />
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-primary">Role</label>
                <select aria-label="Select role" className="w-full h-11 px-4 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/10">
              ...
              <div className="flex gap-2 pt-4">
                <Button type="button" variant="ghost" className="flex-1 rounded-xl h-11" onClick={() => setShowAddUser(false)}>
                  Cancel
                </Button>
        ...
        {showEditProfile && (
          <SettingsModal title="Edit profile" onClose={() => setShowEditProfile(false)}>
            <form className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Button type="button" variant="ghost" className="text-rose-500 font-bold text-xs h-8 px-4">Remove</Button>
                </div>
              </div>
              <div className="flex gap-2 pt-2">
                <Button type="button" variant="ghost" className="flex-1 rounded-xl h-11" onClick={() => setShowEditProfile(false)}>
                  Cancel
                </Button>
        ...
        {showChangePassword && (
          <SettingsModal title="Change Password" onClose={() => setShowChangePassword(false)}>
            <form className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                <Button type="button" variant="ghost" className="flex-1 rounded-xl h-11" onClick={() => setShowChangePassword(false)}>
                  Cancel
                </Button>
                <Button variant="amber" className="flex-1 rounded-xl h-11 font-bold">Save Changes</Button>
              </div>
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center p-6">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button aria-label="Close modal" onClick={onClose} className="text-slate-300 hover:text-primary transition-colors">
            <X size={24} />
          </button>
        </div>
        {children}
