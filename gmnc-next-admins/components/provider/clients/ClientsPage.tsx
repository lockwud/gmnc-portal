...
//   const [data, setData] = React.useState<ClientRecord[]>(clients);
...
//     const matchesRegion = regionFilter === "All Regions" || client.region === regionFilter;
...
//       id: `CP-882-${String(data.length + 1).padStart(3, "0")}`,
...
//           ].map((stat) => (
//             <Card key={stat.label} className="p-5 border-none shadow-sm flex flex-col items-center text-center group hover:shadow-md transition-all">
...
//                   className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-50 rounded-2xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/10 transition-all"
//                   value={searchQuery}
...
//                     {["All Regions", "Greater Accra", "Ashanti Region", "Central Region", "Eastern Region"].map((region) => (
//                       <button
//                         key={region}
...
//         <Modal isOpen={showAddClientModal} onClose={() => setShowAddClientModal(false)} title="Register New Client" className="max-w-md">
//           <form className="space-y-8" onSubmit={handleAddClient}>
//             <div className="space-y-6">
//               <div className="grid grid-cols-2 gap-4">
//                 <Input label="First Name" placeholder="e.g. John" icon={<User size={18} className="text-slate-400" />} value={clientForm.firstName} onChange={(event) => handleClientFormChange("firstName", event.target.value)} required />
//                 <Input label="Last Name" placeholder="e.g. Doe" icon={<User size={18} className="text-slate-400" />} value={clientForm.lastName} onChange={(event) => handleClientFormChange("lastName", event.target.value)} required />
//               </div>
//               <Input label="Email address" type="email" placeholder="email@example.com" icon={<Mail size={18} className="text-slate-400" />} value={clientForm.email} onChange={(event) => handleClientFormChange("email", event.target.value)} required />
//               <div className="grid grid-cols-2 gap-4">
//                 <Input label="Phone Number" placeholder="+233..." icon={<Phone size={18} className="text-slate-400" />} value={clientForm.phone} onChange={(event) => handleClientFormChange("phone", event.target.value)} required />
//                 <div className="space-y-2">
//                   <label className="text-sm font-bold text-primary">Region</label>
//                   <select aria-label="Region" className="w-full h-12 px-4 bg-white border border-slate-200 rounded-2xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-brand/5 focus:border-brand transition-all appearance-none cursor-pointer shadow-sm shadow-slate-200/40" value={clientForm.region} onChange={(event) => handleClientFormChange("region", event.target.value)}>
...
//                 </div>
//               </div>
//               <Input label="Home Address" placeholder="Digital address / Street" icon={<Home size={18} className="text-slate-400" />} value={clientForm.address} onChange={(event) => handleClientFormChange("address", event.target.value)} required />
//             </div>
