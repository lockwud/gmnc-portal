...
import { SearchIcon, DownloadIcon, FileTextIcon, ActivityIcon } from "lucide-react";
      ...
      <div className="space-y-8 pb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input placeholder="Search logs (user, action, IP)..." className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-medium" />
          </div>
            ...
            {["All", "Warning", "Success"].map((type) => (
              <button
                ...
                className={cn(
                  ...
                  filter === type ? "bg-primary text-white" : "text-slate-400 hover:text-slate-600"
                )}
              >
                {type}
              </button>
        ...
        <Table
          ...
          data={MOCK_LOGS.filter((log) => filter === "All" || log.status === filter)}
          columns={[
