            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="p-8 border-none shadow-sm flex flex-col items-center justify-center min-h-[380px] bg-white text-center">
                ...
                <div className="relative w-full h-64 flex items-center justify-center">
                  <ResponsiveContainer width="100%" height={256}>
                    <RadialBarChart innerRadius="80%" outerRadius="100%" data={gaugeData} startAngle={210} endAngle={-30}>
                      <PolarAngleAxis type="number" domain={[0, 100]} angleAxisId={0} tick={false} />
                      <RadialBar background={{ fill: "#eff6ff" }} dataKey="value" cornerRadius={30} fill="#10b981" />
                    </RadialBarChart>
                  </ResponsiveContainer>
              ...
              <Card className="p-8 border-none shadow-sm min-h-[380px] flex flex-col pt-10">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <select aria-label="Improvement range" className="text-[10px] font-bold uppercase tracking-widest text-primary bg-slate-50 px-2 py-1 rounded-lg border-none focus:outline-none">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {patientDistribution.map((patient) => (
                      <tr key={patient.no} className="hover:bg-slate-50/50 transition-colors">
                        <td className="p-4 pl-6 text-xs text-slate-500">{patient.no}</td>
                        <td className="p-4 text-xs font-bold text-primary">{patient.name}</td>
                        <td className="p-4 text-xs text-slate-600">{patient.age}</td>
                        <td className="p-4 text-xs text-slate-600">{patient.sex}</td>
                        <td className="p-4 text-xs text-slate-600">{patient.location}</td>
                        <td className="p-4 pr-6 text-xs font-bold text-emerald-600">{patient.sessions}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <Card className="border-none shadow-sm h-full flex flex-col p-8 pt-10">
              ...
              <div className="space-y-6 flex-1">
                {nonAdherent.map((item, i) => (
                  <div key={`${item.name}-${i}`} className="flex items-center justify-between group cursor-pointer">
